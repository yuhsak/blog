...
Successive Halvingという、ハイパーパラメータの最適化を多腕バンディット問題の一種と捉えて効率的に計画するアルゴリズムを応用したPruner。

[Non-stochastic Best Arm Identification and Hyperparameter Optimization](https://arxiv.org/abs/1502.07943)

これは限られた時間や学習回数の中で出来るだけ良い結果になりそうなパラメータに多くのリソースを割くことに注目したアルゴリズムのよう。
...
数字を大きくすると判定が行われるステップを後回しにできるから判定条件はそのままに長く様子を見る方向に調整するために使える、っていうことかな？
...
どうしてこうすると探索の効率が良くなるのかっていう理論的な部分は正直自分には全然分かってないんだけど、一見するとバランスがとれててリーズナブルに思えるね。
...
SuccessiveHalvingの時に問題だった`リソース`と`生き残らせる割合`のバランスついて様々な組み合わせを用意することでトレードオフを出来るだけ緩和しようとしてる感じ。
...
注意点はこの`max_resource`はあくまでも大体こんなもんですよという数字をPrunerに教えるための参考値だから、例えステップ数がこの数字を超えたとしてもPruner自体がそれを基準に打ち切りをしたりはしないっていうこと。普段通りモデルの学習パラメータでハンドリングしよう。
...
まずそれぞれの子Prunerに対してある方法で求めたバジェットという数字が与えられるんだけど、結果的にあるPrunerが採用される割合は$\frac{\text{そのPrunerのバジェット}}{\text{バジェットの総和}}$となる。
...
| 番号 | 1回目 |   2回目 |   3回目 |   4回目 |    比率 | 生存 |
| :--- | ----: | ------: | ------: | ------: | ------: | ---: |
| 1    |   100 |     200 |     400 |     800 | 36.364% | 1/16 |
| 2    |   200 |     400 |     800 | ~1,600~ | 27.273% |  1/8 |
| 3    |   400 |     800 | ~1,600~ | ~3,200~ | 18.182% |  1/4 |
| 4    |   800 | ~1,600~ | ~3,200~ | ~6,400~ | 18.182% |  1/2 |
...
| 番号 | 1回目 |   2回目 |   3回目 |    比率 | 生存 |
| :--- | ----: | ------: | ------: | ------: | ---: |
| 1    |   100 |     300 |     900 | 52.941% | 1/27 |
| 2    |   300 |     900 | ~2,700~ | 29.412% |  1/9 |
| 3    |   900 | ~2,700~ | ~8,100~ | 17.647% |  1/3 |
...
| 番号 | 1回目 |   2回目 |    比率 | 生存 |
| :--- | ----: | ------: | ------: | ---: |
| 1    |   100 |     400 | 66.667% | 1/16 |
| 2    |   400 | ~1,600~ | 33.333% |  1/4 |

表にしてみると分かりやすいね。

作成されるPruner数の傾向はこんな感じかな。

- `max_resource`に対する`min_resource`の割合が小さいほどたくさん作られる
- `reduction_factor`が小さいほど子Prunerがたくさん作られる

どのパターンでも生き残る割合が異なったPrunerにバランスよく配分されるようになってるからある程度SuccessiveHalvingのトレードオフを緩和できていそうだね。

ただ基本的にはやはり`reduction_factor`が大きいほど少ないリソースを許容するようになって逆に小さいほど求められる総リソースが増えるというSuccessiveHalvingの特徴は踏襲していると思うから、そこらへん考えて調整する方針で良さそう。

1. まず個々のTrialに最低限与えたいステップ数と許容できる最大のステップ数を決める (→ ベースのPruner数が決まる)
2. 確保できるリソースを加味して`reduction_factor`を調整する

例によってあんまり自信が無いんだけどね。

SuccessiveHalving単体よりもうまくいきそうな感じがすごいね…！

...
export const Counter = () => {
  ...
  return <button onClick={handle}>{count} クリック</button>
}
...
当初はどうせマークダウンで書くなら独自仕様を避けたプレーンな形式にしておけば例えば後から簡単にObsidianに移行したりも出来るからなるべくそうしようと思ってたんだけど、よく考えたらブログ記事をObsidianに移行することって多分永遠にないよね🤔？笑
...
万が一記事中にクリックカウンターを表示したくなった時だってMDXならこの通り…!!
...
`gatsby-transformer-remark`を`gatsby-plugin-mdx`に差し替えて、`allMarkdownRemarks` → `allMdx` みたくプロパティ名変わるとこ変えて、remarkの`plugins`に入れてたやつらをmdxの`gatsbyRemarkPlugins`に引っ越す。ほぼほぼこれだけ。

ただ1点だけ、数式の表示に使っていた`gatsby-remark-katex`だけはどうしてもmdxと一緒に動かすことができず。
...
`gatsby-remark-katex`を使うのをやめて`remark-math`と`rehype-katex`を使うと上手くいく。
...
ちなみに肝心の`katex`は`rehype-katex`のdependencyとして一緒にくっついてくるので`gatsby-remark-katex`の時みたいに明示的にインストールしなくても大丈夫。
...
原因は`gatsby-plugin-mdx`で利用している`@mdx-js/mdx@1.0.0`で使われてるremarkが1つ前の12系だから。

...
ブラウザ閉じても回り続けてくれるのが便利すぎて自分もめっちゃヘビーユーズしてる。
...
これは個人用のGoogleアカウントの支払い設定とは全く別のもので既に個人のアカウントに支払い情報を設定済みでもGCP側であらためて設定する必要がある。

例えばFirebaseの無料プランみたいに特に支払いの設定をしなくても使える機能も色々あるんだけど、原則課金が発生する可能性がある機能はそもそも支払いの設定が済んでいないと使い始めることができないみたい。コラボのカスタムVMもそう。

GCPは初めて利用を開始する時に3万円分の無料クレジットをくれるからカスタムVMをちょっと試すだけなら実際に課金が発生することはないんだけどね。利用を開始するために支払いの設定自体は必要。
...
自分のユースケースに沿った内容にしよう。デフォルトだとコラボの無料版とだいたい同定程度に設定されている感じだった。  
...
| ストレージ            |       1,499円 |       2.082円 |
| GPU (T4の場合)        |      29,463円 |      40.921円 |
| GPU (P100の場合)      |     122,903円 |     170.699円 |
| **合計 (T4の場合)**   |  **50,879円** |  **70.665円** |
| **合計 (P100の場合)** | **144,319円** | **200.443円** |
...
| ストレージ            |       1,499円 |       2.082円 |
| GPU (P100の場合)      |     122,903円 |     170.699円 |
| GPU                   |     208,766円 |     289.953円 |
| **合計 (P100の場合)** | **164,237円** | **228.107円** |
| **合計 (V100の場合)** | **250,100円** | **347.361円** |
...
次回使うときはセッションの接続前に再開ボタンを押してVMを起動することを忘れずに。
...
コラボのUIにブラウザを介してVMにファイルのアップロード/ダウンロードをする機能があるにはあるんだけど大きなサイズのファイルは無理だし、このままではファイルのやり取りが不便だからCloud Storageのバケットをファイルシステムとしてマウントすることで代替しよう。

AWSのS3が有名だけどGCPにもGoogle Cloud Storageという同様のオブジェクトストレージサービスがある。

オブジェクトストレージというのはその名の通り何でもファイルを保存しておける便利なサービスで、課金体系はファイルの出し入れと保存してる容量に対して使った分だけの従量課金になってることが多い系のやつなんだけど、もし何のことか全然わからなかったらちょっとググって調べてみてね。

そんでこの類のオブジェクトストレージは大抵ファイルシステムとしてマウントできるようにするOSSがある。外付けHDDみたいに使えるようにするやつ。

GCS用には[gcsfuse](https://github.com/GoogleCloudPlatform/gcsfuse)というのがあるのでこれを使ってVMにマウントすることでGoogle Driveの代わりにするというわけ。
...
アクセス権を変更するにはVMが停止状態の必要があるからもし起動中だったら停止してから「編集」ボタンを押して設定画面を開こう。

下の方にちょっとスクロールすると「セキュリティ&アクセス」という項目の中に「Identity and API access」という項目があって、アクセススコープが「APIごとにアクセス権を設定」になっていると思う。

ここのストレージの項目は初期設定だと「読み取りのみ」になっていると思うからこれを「フル」に変更しよう。
...
ここまで出来たらVMを起動してNotebookのセッションを接続しよう。

まずセルから下記のコマンドを実行してgcsfuseをインストールする。
...
続いてバケットをマウント。GCSのバケット名は `my-bucket` になってるとして、下記のコマンドを実行するとマウントできる。
...
正常に実行できたら試しに `!echo test > /content/gcs/test.txt` とかやってみてGCSに書き込まれるか確認してみよう。

うまくいっていればGCP側のバケットの画面でtest.txtが作成されていることが確認できるはず。
...
2. からの企業が大規模に運用している本格的な分析ワークロードへのシームレスな接続体験を提供することで
